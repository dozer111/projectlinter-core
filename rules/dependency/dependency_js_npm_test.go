...
func TestJavascriptNPMDependencySet(t *testing.T) {
	pathProvider := path_provider.NewPathProvider(utilTest.PathInProjectLinter("testdata/js_npm"))

	s := dependency.NewJavascriptNPMDependencySet(pathProvider, nil, nil)
	errs := s.Init()
